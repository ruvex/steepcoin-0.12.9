...
#if defined(NDEBUG)
# error "Arc Core cannot be compiled without assertions."
#endif
...
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
    ...
    if (nPrevHeight ==0) {
        nSubsidyBase = 20060000; //20060000  всего монет за 1 год работы и к моменту перехода на новую версию кошелька.
		LogPrintf("First block: Transfer members balance %i ARC\n", nSubsidyBase);
		/* 
...
int32_t ComputeBlockVersion(const CBlockIndex* pindexPrev, const Consensus::Params& params, bool fAssumeGoldminenodeIsUpgraded)
{
    LOCK(cs_main);
    ...
    for (int i = 0; i < (int)Consensus::MAX_VERSION_BITS_DEPLOYMENTS; i++) {
        ...
        if (vbinfo.check_mn_protocol && state == THRESHOLD_STARTED && !fAssumeGoldminenodeIsUpgraded) {
            CScript payee;
            ...
            if (!mnodeman.GetGoldminenodeInfo(payee, mnInfo)) {
                // unknown goldminenode
                continue;
            }
            if (mnInfo.nProtocolVersion < DIP0001_PROTOCOL_VERSION) {
                // goldminenode is not upgraded yet
                continue;
            }
        }
